...
func TestPgxIntegration(t *testing.T) {
	...
	// Guest identity against real Postgres: create, convert, purge (FK cascade).
	...
	gid := guest.ID
	if err := um.ConvertToRegistered(ctx, guest, "pg_promoted", "promoted@x.com", "Abcdef1!"); err != nil {
